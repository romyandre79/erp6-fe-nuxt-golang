...
<template>
  <div class="relative min-h-screen pb-20 overflow-hidden">
     <!-- Background Accents -->
    <div class="absolute top-0 right-0 w-[600px] h-[600px] bg-emerald-500/5 rounded-full blur-[120px] pointer-events-none"></div>
    <div class="absolute bottom-0 left-0 w-[500px] h-[500px] bg-cyan-500/5 rounded-full blur-[100px] pointer-events-none"></div>
    ...
    <div class="max-w-4xl mx-auto px-4 pt-12 relative z-10">
      ...
      <div class="flex justify-between items-center mb-12 relative">
        <div class="absolute top-1/2 left-0 w-full h-0.5 bg-slate-800 -z-10 transform -translate-y-1/2"></div>
         <div class="absolute top-1/2 left-0 h-0.5 bg-emerald-500 -z-10 transform -translate-y-1/2 transition-all duration-500" :style="{ width: `${((currentStep - 1) / (steps.length - 1)) * 100}%` }"></div>
      ...
      <div class="bg-slate-800/50 backdrop-blur-md border border-white/5 rounded-2xl p-8 md:p-10 shadow-xl">
        ...
        <div v-if="currentStep === 1" class="space-y-6 animate-fade-in">
          ...
          <div class="grid md:grid-cols-2 gap-6">
            <div class="space-y-2">
              ...
              <input v-model="formData.companyName" type="text" class="w-full bg-slate-900/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 transition-colors" placeholder="Legal Entity Name" />
            </div>
             <div class="space-y-2">
              <label class="text-sm font-medium text-slate-400">Registration Number</label>
              <input v-model="formData.registrationNumber" type="text" class="w-full bg-slate-900/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 transition-colors" placeholder="Business Reg. No." />
            </div>
             <div class="space-y-2 md:col-span-2">
              <label class="text-sm font-medium text-slate-400">Registered Address</label>
              <textarea v-model="formData.address" rows="3" class="w-full bg-slate-900/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 transition-colors" placeholder="Full Business Address"></textarea>
            </div>
            <div class="space-y-2">
              <label class="text-sm font-medium text-slate-400">Contact Person</label>
               <input v-model="formData.contactName" type="text" class="w-full bg-slate-900/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 transition-colors" placeholder="Full Name" />
            </div>
             <div class="space-y-2">
              <label class="text-sm font-medium text-slate-400">Email Address</label>
               <input v-model="formData.email" type="email" class="w-full bg-slate-900/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 transition-colors" placeholder="contact@company.com" />
            </div>
          </div>
        </div>
        ...
        <div v-if="currentStep === 2" class="space-y-6 animate-fade-in">
           ...
           <div class="grid md:grid-cols-2 gap-6">
             <div class="space-y-2">
              ...
              <input v-model="formData.taxId" type="text" class="w-full bg-slate-900/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 transition-colors" placeholder="Tax Identification Number" />
            </div>
            <div class="space-y-2">
              <label class="text-sm font-medium text-slate-400">Business Type</label>
              <select v-model="formData.businessType" class="w-full bg-slate-900/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 transition-colors">
        ...
        <div v-if="currentStep === 3" class="space-y-6 animate-fade-in">
           ...
           <div class="grid md:grid-cols-2 gap-6">
             <div class="border-2 border-dashed border-slate-700/50 rounded-xl p-8 text-center hover:border-emerald-500/50 hover:bg-emerald-500/5 transition-colors cursor-pointer group">
        ...
        <div v-if="currentStep === 4" class="space-y-6 animate-fade-in">
           ...
           <div class="flex items-center gap-3 pt-4">
             <input v-model="formData.termsAccepted" type="checkbox" id="terms" class="w-5 h-5 rounded border-slate-600 bg-slate-700 text-emerald-500 focus:ring-emerald-500/50 transition-colors">
             <label for="terms" class="text-slate-400 text-sm">I accept the <a href="#" class="text-emerald-400 hover:underline">Terms and Conditions</a> and <a href="#" class="text-emerald-400 hover:underline">Privacy Policy</a>.</label>
           </div>
        </div>
