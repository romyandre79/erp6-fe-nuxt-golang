  ...
  <div class="min-h-screen bg-slate-900 pt-24 pb-12 px-4">
    <div class="max-w-7xl mx-auto">
      ...
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10">
        ...
        <button
          class="px-5 py-2.5 bg-slate-800 hover:bg-slate-700 text-white rounded-lg border border-white/5 font-medium transition-colors flex items-center gap-2"
        >
      ...
      <div class="flex flex-col lg:flex-row gap-8">
          ...
          <div class="bg-slate-800/50 rounded-xl p-5 border border-white/5">
            ...
            <div class="relative">
              <input
                v-model="searchQuery"
                type="text"
                class="w-full bg-slate-900 border border-white/10 rounded-lg pl-9 pr-4 py-2.5 text-sm text-white focus:outline-none focus:border-emerald-500/50 transition-colors"
                placeholder="Keywords..."
              />
              <svg
                class="w-4 h-4 text-slate-500 absolute left-3 top-3"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <div class="space-y-2">
              <label v-for="cat in categories" :key="cat" class="flex items-center gap-3 cursor-pointer group">
                <input
                  ...
                  class="w-4 h-4 text-emerald-500 bg-slate-900 border-slate-600 focus:ring-emerald-500/50 focus:ring-offset-0"
                />
            ...
            <div class="space-y-2">
              ...
              <label class="flex items-center gap-3 cursor-pointer group">
                <input
                  ...
                  class="w-4 h-4 text-emerald-500 bg-slate-900 border-slate-600 focus:ring-emerald-500/50 focus:ring-offset-0"
                />
                <span class="text-slate-400 group-hover:text-white text-sm transition-colors"
                  >Urgent / Closing Soon</span
                >
              </label>
            </div>
          </div>
        ...
        <div class="flex-grow space-y-4">
          <div
            ...
            class="text-center py-20 bg-slate-800/30 rounded-xl border border-dashed border-slate-700"
          >
          ...
          <div
            ...
            :key="tender.id"
            class="group bg-slate-800/50 hover:bg-slate-800 p-6 rounded-xl border border-white/5 hover:border-emerald-500/30 transition-all hover:shadow-lg hover:shadow-emerald-900/10 flex flex-col md:flex-row gap-6"
          >
            <div class="flex-grow">
              <div class="flex items-start gap-4 mb-2">
                <span
                  class="px-2.5 py-0.5 bg-slate-700/50 rounded text-xs font-semibold text-slate-300 border border-white/5"
                  >{{ tender.category }}</span
                >
                <span
                  v-if="tender.status === 'Urgent'"
                  class="px-2.5 py-0.5 bg-amber-500/10 text-amber-400 rounded text-xs font-semibold border border-amber-500/20"
                  >Urgent</span
                >
              </div>
            ...
            <div class="flex flex-col justify-center shrink-0">
              <NuxtLink
                :to="`/e-procurement/tenders/${tender.id}`"
                class="px-6 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-bold rounded-lg text-center transition-colors"
              >
                View Details
              </NuxtLink>
            </div>
