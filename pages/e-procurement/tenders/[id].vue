...
const tender = computed(() => {
  return {
    ...
    deadline: '2025-12-25',
    status: 'Open',
    description: 'We are seeking a qualified vendor to upgrade our core IT infrastructure across 5 regional offices. The project includes network hardware replacement, server consolidation, and implementation of a new secure Wi-Fi mesh system.',
    requirements: [
        ...
        <div class="flex flex-col md:flex-row justify-between gap-6 relative z-10">
          <div>
            <div class="flex items-center gap-3 mb-3">
              <span class="px-3 py-1 bg-slate-700/50 rounded-full text-xs font-semibold text-slate-300 border border-white/5">{{ tender.category }}</span>
          ...
          <div class="flex flex-col gap-3 shrink-0">
            <button class="px-8 py-3 bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-bold rounded-lg shadow-lg shadow-emerald-500/20 transition-all hover:-translate-y-0.5">
              Apply Now
            </button>
            <button class="px-8 py-3 bg-slate-700 hover:bg-slate-600 text-white font-semibold rounded-lg transition-colors border border-white/5">
              Download Docs
            </button>
          </div>
        </div>
          ...
          <div class="bg-slate-800/50 rounded-xl p-8 border border-white/5">
            ...
            <ul class="space-y-3">
              <li v-for="(req, index) in tender.requirements" :key="index" class="flex items-start text-slate-300">
                <svg class="w-5 h-5 text-emerald-500 mr-3 mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                <span>{{ req }}</span>
              </li>
            </ul>
          </div>
        ...
        <div class="space-y-6">
           <div class="bg-slate-800/50 rounded-xl p-6 border border-white/5">
             ...
             <div class="space-y-6 relative ml-2">
               ...
               <div v-for="(item, index) in tender.timeline" :key="index" class="relative pl-6">
                 <div class="absolute left-[-5px] top-1.5 w-3 h-3 rounded-full bg-slate-900 border-2" :class="index === 0 ? 'border-emerald-500 bg-emerald-500' : 'border-slate-500'"></div>
                 <div class="text-sm text-slate-400 mb-0.5">{{ item.date }}</div>
                 <div class="font-medium text-white" :class="index === 0 ? 'text-emerald-400' : ''">{{ item.event }}</div>
               </div>
             </div>
           </div>
           ...
           <div class="bg-gradient-to-br from-indigo-900/40 to-slate-900/40 rounded-xl p-6 border border-indigo-500/20">
             ...
             <button class="text-indigo-400 text-sm font-medium hover:text-indigo-300 flex items-center">
               Contact Support <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
             </button>
           </div>
        </div>
