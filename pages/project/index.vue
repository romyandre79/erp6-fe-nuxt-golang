...
<template>
  <div class="h-full flex flex-col">
    ...
    <div v-if="!userStore.isLoggedIn" class="flex-1 flex overflow-hidden">
      <!-- Hero Section -->
      <div
        class="flex-1 bg-gradient-to-br from-slate-900 to-slate-800 text-white p-12 flex flex-col justify-center relative overflow-hidden"
      >
        <div
        ...
        <div class="relative z-10 max-w-2xl">
          ...
          <h1 class="text-5xl font-bold mb-6 leading-tight">
            Manage your projects with <span class="text-blue-400">clarity</span> and
            <span class="text-purple-400">style</span>.
          </h1>
          <p class="text-lg text-gray-300 mb-8 leading-relaxed">
            The ultimate tool for agile teams. Visualize efficient workflows, track progress in real-time, and
          ...
          <div class="flex gap-4">
            <div class="flex -space-x-4">
              ...
              <div
                class="flex items-center justify-center w-8 h-8 rounded-full border-2 border-slate-900 bg-gray-700 text-xs text-white z-10"
              >
                +2k
              </div>
            </div>
      ...
      <div class="w-full max-w-md bg-white dark:bg-gray-800 p-12 flex flex-col justify-center overflow-y-auto">
        ...
        <div v-else class="text-center">
          <div
            class="w-16 h-16 bg-green-100 dark:bg-green-900/30 text-green-600 rounded-full flex items-center justify-center mx-auto mb-4"
          >
          ...
          <UButton to="/auth/login" color="gray" variant="ghost"> Return to Login </UButton>
        </div>
      </div>
    </div>
    ...
    <div v-else class="flex flex-1 overflow-hidden">
      ...
      <div class="flex-1 flex flex-col overflow-hidden bg-blue-50/50 dark:bg-gray-900">
        <!-- Board Header -->
        <div
          class="h-14 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center px-4 justify-between"
        >
        ...
        <div class="flex-1 overflow-x-auto overflow-y-hidden p-6">
          <div class="flex h-full gap-6">
            <div v-for="column in columns" :key="column.status" class="w-80 flex-shrink-0 flex flex-col max-h-full">
              <!-- Column Header -->
              <div
                class="bg-gray-100 dark:bg-gray-800 p-3 rounded-t-lg border-b border-gray-200 dark:border-gray-700 flex items-center justify-between"
              >
              ...
              <div class="bg-gray-100 dark:bg-gray-800 p-2 rounded-b-lg flex-1 overflow-y-auto space-y-2 min-h-0">
                ...
                >
                  <div class="flex items-start justify-between mb-2">
                    <UBadge
                      v-if="card.priority"
                      :color="card.priority === 'high' ? 'red' : 'blue'"
                      size="xs"
                      variant="subtle"
                      >{{ card.priority }}</UBadge
                    >
                  </div>
                ...
                <UButton block variant="ghost" color="gray" icon="i-heroicons-plus" class="mt-2 text-gray-500"
                  >Add Card</UButton
                >
              </div>
            </div>
            ...
            <div class="w-80 flex-shrink-0">
              <UButton block color="gray" variant="soft" icon="i-heroicons-plus" class="h-10 bg-white/50 backdrop-blur"
                >Add another list</UButton
              >
            </div>
          </div>
        </div>
