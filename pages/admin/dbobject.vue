    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <div class="flex-1 relative p-4 bg-gray-100" @dragover.prevent @drop="onDropCanvas" ref="canvasRef">
        ...
        <svg class="absolute inset-0 pointer-events-none" style="overflow: visible;">
          ...
          <path v-if="linkPreview.active" :d="linkPreview.path" stroke="gray" stroke-width="2" fill="none" stroke-dasharray="6 4" />
        </svg>
      ...
      <aside class="w-96 border-l bg-white p-4 overflow-auto">
        <div v-if="selectedTable" class="space-y-4">
          ...
          <div>
            ...
            <textarea v-model="jsonPreview" rows="6" class="w-full p-2 border rounded mt-1 text-xs font-mono"></textarea>
        ...
        <div>
          ...
          <pre class="mt-2 text-xs bg-gray-50 p-2 border rounded max-h-44 overflow-auto font-mono">{{ JsonExport }}</pre>
        </div>
...
function addTableAt(x, y) { tables.push(makeTable(x, y)) }
...
function addColumn() { if (selectedTable.value) selectedTable.value.columns.push({ name: 'new_col', type: 'varchar' }) }
function removeColumn(idx) { if (selectedTable.value) selectedTable.value.columns.splice(idx, 1) }
...
function copyJSONToClipboard() { navigator.clipboard.writeText(jsonPreview.value).then(() => alert('Copied')) }
  ...
  reader.onload = () => {
    try {
      ...
      if (obj && Array.isArray(obj.tables)) {
        ...
        relations.splice(0, relations.length, ...(Array.isArray(obj.relations) ? obj.relations.map(r => ({ ...r })) : []))
...
function finishLink(ev, moveHandler, upHandler) {
  ...
  if (target) {
    const rel = {
      id: relSeq++,
      from: { table: linkPreview.from.table, col: linkPreview.from.col, colName: tables.find(t=>t.id===linkPreview.from.table).columns[linkPreview.from.col].name },
      to: { table: target.table, col: target.col, colName: tables.find(t=>t.id===target.table).columns[target.col].name },
      path: ''
    }
    relations.push(rel)
    computeRelationsPaths()
...
function removeRelation(id) { const idx = relations.findIndex(r => r.id === id); if (idx >= 0) relations.splice(idx,1) }
function editRelation(id) { alert('Edit relation not implemented yet') }
