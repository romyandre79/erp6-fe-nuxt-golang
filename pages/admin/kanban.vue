<template>
  <div
    class="flex h-[calc(100vh-64px)] border-t"
    style="background: var(--body-background); border-color: var(--border-color)"
  >
    <!-- Mobile Sidebar Overlay -->
    <div v-if="isSidebarOpen" class="fixed inset-0 bg-black/50 z-20 md:hidden" @click="isSidebarOpen = false"></div>
    ...
    >
      ...
      <div class="p-4 border-b" style="border-color: var(--border-color)">
        ...
        <UButton block color="primary" icon="i-heroicons-plus" size="sm" @click="isTemplateModalOpen = true">
          New Project
        </UButton>

        <!-- Archive Toggle -->
        <div class="mt-3 flex items-center justify-between">
          <label class="text-sm cursor-pointer" @click="showArchived = !showArchived"> Show Archived </label>
          <input
            type="checkbox"
        ...
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2 flex-1 min-w-0">
              ...
              <UIcon v-if="project.archived" name="i-heroicons-archive-box" class="w-3 h-3 flex-shrink-0" />
            </div>
    ...
    <div class="flex-1 flex flex-col overflow-hidden" ref="mainContentRef">
      ...
      <div v-if="activeProject" class="flex flex-col flex-1 overflow-hidden">
        ...
        >
          <div
            ...
            style="background: var(--button-background); border: 1px solid var(--border-color)"
          >
            <UButton
            ...
            <div v-if="activeProject" class="flex gap-2">
              <UButton icon="i-heroicons-chart-bar" color="blue" variant="soft" @click="openStatisticsModal">
                Statistics
              </UButton>
              <UButton icon="i-heroicons-users" color="purple" variant="soft" @click="openMembersModal">
                Members
              </UButton>
              <UButton icon="i-heroicons-cog-6-tooth" color="gray" variant="soft" @click="openColumnManager">
                Manage Columns
              </UButton>
              <UButton icon="i-heroicons-photo" color="gray" variant="soft" @click="exportView('png')"> PNG </UButton>
              <UButton icon="i-heroicons-document-text" color="gray" variant="soft" @click="exportView('pdf')">
                PDF
              </UButton>
              <UButton
        ...
        >
          ...
          <div class="h-full">
            <div class="kanban-board w-full overflow-x-auto h-full" style="display: block">
              <div
                class="flex flex-row gap-4 pb-4"
                style="display: flex; flex-direction: row; min-height: 100%; flex-wrap: nowrap"
              >
                <!-- Dynamic Columns -->
                <div
                  ...
                  >
                    <template #header>
                      ...
                      >
                        <div class="flex items-center gap-2">
                          ...
                          <UIcon
                            v-if="hasColumnRestrictions(column)"
                    ...
                    >
                      ...
                      >
                        ...
                        <div
                          class="flex items-center justify-between text-xs pt-2 border-t border-gray-200 dark:border-gray-700"
                        >
                          <div v-if="card.assignee" class="flex items-center gap-1">
                            <UIcon name="i-heroicons-user-circle" class="w-4 h-4" />
                            <span>{{
                              activeProject?.members?.find(
                                (m: any) => m.userid == card.assignee || m.email == card.assignee,
        ...
        <div
          ...
          ref="listViewRef"
          class="h-full flex flex-col rounded-lg shadow-sm border overflow-hidden"
          style="background: var(--panel-background); border-color: var(--border-color)"
        >
          <!-- List Header -->
          <div class="px-6 py-4 border-b flex items-center justify-between" style="border-color: var(--border-color)">
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="flex items-center gap-2">
                ...
                <select v-model="listGroupBy" class="text-sm border-gray-300 dark:border-gray-600 rounded-md py-1">
          ...
          <div class="flex-1 overflow-auto p-4 space-y-6">
            <div v-for="(groupTasks, groupName) in groupedTasks" :key="groupName" class="space-y-2">
              ...
              <!-- Task Table -->
              <div
                v-if="!groupTasks._collapsed"
                class="border rounded-lg shadow-sm overflow-x-auto"
                style="background: var(--panel-background); border-color: var(--border-color)"
              >
                <!-- Table Header (Optional, mostly implied) -->
                <div
                  class="grid grid-cols-12 gap-4 px-4 py-2 border-b text-xs font-semibold text-gray-500 min-w-[700px]"
                        ...
                        <template v-if="task.tags?.length">
                          <span class="inline-flex gap-1">
                            <span
                              ...
                              :key="tag"
                              class="px-1.5 py-0.5 rounded text-[10px] font-medium bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-300"
                            >
                              {{ tag }}
                            </span>
                  ...
                  <div class="col-span-2">
                    <div class="flex text-sm items-center gap-1.5">
                      <div
                        ...
                        :style="{ backgroundColor: getPriorityColorHex(task.priority) }"
                      ></div>
                  ...
                  <div class="col-span-2 flex items-center justify-end pr-2">
                    ...
                    >
                      <UAvatar
                        :alt="
                          activeProject?.members?.find(
                            (m: any) => m.userid == task.assignee || m.email == task.assignee,
                ...
                <div
                  ...
                  @click="
                    openCreateModal(
                      listGroupBy === 'status' ? columns.find((c) => c.title === groupName)?.status || 'todo' : 'todo',
                    )
                  "
                >
          ...
          <div class="flex flex-1 overflow-hidden">
            ...
            <div class="flex-1 overflow-auto flex flex-col">
              ...
              <div v-if="calendarViewMode === 'week'" class="flex-1 flex flex-col min-w-[800px]">
                <!-- Header Row -->
                <div
                  class="grid grid-cols-8 border-b"
                  style="background: var(--table-head-background); border-color: var(--border-color)"
                >
                  <div
                    class="p-3 text-sm font-semibold sticky left-0 z-10 border-r"
                    style="
                  ...
                  >
                    ...
                    <div
                      ...
                      :class="{ 'text-primary-600': day.toDateString() === new Date().toDateString() }"
                    >
                      {{ day.getDate() }}
                    </div>
                  </div>
                ...
                <div class="flex-1 overflow-y-auto">
                  ...
                  >
                    ...
                    <div
                      class="p-3 border-r sticky left-0 z-10 flex flex-col items-center justify-center gap-2"
                      style="background: var(--panel-background); border-color: var(--border-color)"
                    >
                      ...
                      <template v-else>
                        <div
                          class="w-10 h-10 rounded-full bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center text-primary-600 dark:text-primary-400 font-bold border border-primary-200 dark:border-primary-800"
                        >
              ...
              <div v-else-if="calendarViewMode === 'month'" class="flex-1 p-4 overflow-auto">
                <div
                  class="grid grid-cols-7 gap-px rounded-lg overflow-hidden h-full min-w-[700px]"
                ...
                >
                  ...
                  >
                    <div
                      class="text-right text-xs p-1 font-medium text-gray-500"
                      :class="{ 'text-primary-600 font-bold': day.date.toDateString() === new Date().toDateString() }"
                    >
                      {{ day.day }}
                    </div>
              ...
              <div
                ...
                class="flex-1 p-4 flex justify-center"
                style="background: var(--body-background)"
              >
                <div
                  class="w-full max-w-2xl rounded shadow-sm border flex flex-col"
                  style="background: var(--panel-background); border-color: var(--border-color)"
                >
                  ...
                  <div class="p-4 space-y-3 flex-1 overflow-y-auto">
                    ...
                    >
                      <div
                        ...
                        :style="{ backgroundColor: getPriorityColorHex(card.priority) }"
                      ></div>
                      <div class="flex-1">
                      ...
                      <div v-if="card.assignee" class="flex items-center gap-2">
                        <UAvatar
                          :alt="activeProject?.members?.find((m: any) => m.userid == card.assignee)?.username"
                          size="sm"
                        />
                      </div>
                    </div>
            ...
            <div
              class="w-80 border-l border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 flex flex-col"
            >
        ...
        >
          <!-- Gantt Header -->
          <div class="px-6 py-4 border-b flex items-center justify-between" style="border-color: var(--border-color)">
            <h3 class="text-xl font-bold">Project Timeline</h3>
            <div class="text-sm text-gray-500">{{ ganttTotalDays }} days</div>
          </div>
          ...
          <div class="flex-1 overflow-auto flex">
            ...
            <div class="sticky left-0 flex z-30" style="background: var(--panel-background)">
              ...
              <div
                class="w-64 flex-shrink-0 border-r"
                style="background: var(--panel-background); border-color: var(--border-color)"
              >
                <div
                  class="h-10 border-b px-4 flex items-center text-sm font-semibold sticky top-0 z-20"
                  style="
                    background: var(--table-head-background);
                    border-color: var(--border-color);
                    color: var(--table-head-color);
              ...
              <div
                class="w-32 flex-shrink-0 border-r z-10"
                style="background: var(--panel-background); border-color: var(--border-color)"
              >
                <div
                  class="h-10 border-b px-4 flex items-center text-sm font-semibold sticky top-0 z-20"
            ...
            <div class="flex-1">
              <div class="min-w-max">
                <!-- Days Header -->
                <div
                  class="h-10 border-b grid sticky top-0 z-20"
                  :style="`grid-template-columns: repeat(${ganttTotalDays}, 32px); background: var(--table-head-background); border-color: var(--border-color);`"
                >
                  <div
                ...
                <div class="relative">
                  ...
                  <div
                    ...
                    :style="`grid-template-columns: repeat(${ganttTotalDays}, 32px)`"
                  >
                    <div
                      v-for="(day, index) in ganttDays"
                      :key="index"
                  ...
                  <div
                    ...
                    :key="task.id"
                    class="h-10 border-b border-gray-100 dark:border-gray-800 relative grid items-center"
                    :style="`grid-template-columns: repeat(${ganttTotalDays}, 32px)`"
                  >
                    <div
      ...
      <div v-else class="flex-1 flex items-center justify-center">
        <div class="text-center">
          ...
          <UButton color="primary" @click="openProjectModal(null)"> Create Your First Project </UButton>
        </div>
      </div>
    </div>
  </div>
      ...
      <UCard style="background: var(--panel-background); border: 1px solid var(--border-color)">
        <template #header>
          <div class="flex items-center justify-between">
            ...
            <UButton icon="i-heroicons-x-mark" color="gray" variant="ghost" size="sm" @click="closeProjectModal" />
          </div>
        </template>
          ...
          <div>
            ...
            <div class="flex gap-2">
              <div
                ...
                :class="
                  editingProject.color === color ? 'border-gray-900 dark:border-white scale-110' : 'border-transparent'
                "
        ...
        <template #footer>
          <div class="flex justify-end gap-2">
            <UButton color="gray" variant="ghost" @click="closeProjectModal"> Cancel </UButton>
        ...
        <div class="space-y-4">
          ...
          <div class="space-y-2">
            ...
            >
              ...
              <!-- Title Input -->
              <UInput v-model="column.title" placeholder="Column Title" size="sm" class="flex-1" />

              <!-- Status Key Input -->
              <UInput v-model="column.status" placeholder="status_key" size="sm" class="w-32" />
              ...
              <div class="flex gap-1">
                ...
                <UButton
                  ...
                  @click="moveColumn(index, 1)"
                />
                <UButton icon="i-heroicons-trash" size="sm" color="red" variant="ghost" @click="deleteColumn(index)" />
              </div>
            </div>
          </div>
          ...
          <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
            <UButton block color="primary" variant="soft" icon="i-heroicons-plus" @click="addColumn">
              Add Column
            </UButton>
          </div>
        </div>
        ...
        <div class="grid grid-cols-12 gap-6 p-6" :key="editingCard.id">
          ...
          <div class="col-span-9 space-y-6">
            ...
            <div>
              <div class="flex items-center justify-between mb-3">
                ...
                <label class="cursor-pointer">
                  <UButton icon="i-heroicons-plus" size="sm" color="gray" variant="soft" as="span"> Add </UButton>
            ...
            <div>
              <div class="flex items-center justify-between mb-3">
                ...
                <UButton icon="i-heroicons-plus" size="sm" color="primary" variant="soft" @click="addTimeEntry">
                  Add Entry
                </UButton>
              </div>
              ...
              <div v-if="editingCard.timeEntries && editingCard.timeEntries.length > 0" class="space-y-3">
                ...
                >
                  ...
                  <!-- Note -->
                  <div class="mb-2">
                    <label class="block text-xs font-medium mb-1"> Note (optional) </label>
                    <input
                      v-model="entry.note"
                      type="text"
            ...
            <div>
              ...
              <div class="space-y-3">
                <div class="flex gap-3">
                  ...
                  <div class="flex-1">
                    <UTextarea v-model="newComment" placeholder="Write a comment..." :rows="2" class="w-full" />
                ...
                <div v-if="editingCard.comments && editingCard.comments.length > 0" class="space-y-4 pt-2">
                  <div
                    v-for="(comment, idx) in editingCard.comments"
                    :key="comment.cardcommentid || idx"
                    class="flex gap-3 group"
                  >
                    <!-- Avatar -->
                    <div
                      class="w-8 h-8 rounded-full bg-blue-500 flex items-center justify-center flex-shrink-0 text-white text-xs font-semibold"
                    >
                      {{
                        (
                          activeProject?.members?.find((m: any) => m.userid == comment.userid)?.username ||
                          comment.userid ||
                          'U'
                        )
                          .charAt(0)
          ...
          <div class="col-span-3 space-y-4">
            ...
            <div>
              ...
              <div class="space-y-3">
                ...
                <div>
                  ...
                  <div
                    v-if="editingCard.assignee"
                    class="mt-2 flex items-center gap-2 p-2 rounded"
                    style="background: var(--body-background); border: 1px solid var(--border-color)"
                  >
                    <!-- Handle object/array returned by component -->
                    <template v-if="typeof editingCard.assignee === 'object'">
                      <div
                        class="w-6 h-6 rounded-full bg-blue-500 flex items-center justify-center text-white text-xs font-semibold"
                      >
                        {{
                          (
                            (Array.isArray(editingCard.assignee) ? editingCard.assignee[0] : editingCard.assignee) || {}
                          ).label
                            ?.charAt(0)
                            .toUpperCase() || 'U'
                        }}
                    ...
                    <template v-else>
                      ...
                      >
                        {{
                          (
                            activeProject?.members?.find(
                              (m: any) => m.userid == editingCard.assignee || m.email == editingCard.assignee,
                            )?.username || String(editingCard.assignee)
                          )
                            .charAt(0)
                            .toUpperCase()
                        }}
                ...
                <div>
                  ...
                  <UInput v-model="tagsInput" placeholder="tag1, tag2, tag3" size="sm" @blur="updateTags" />
                  <!-- Show existing tags -->
            ...
            <div>
              ...
              <div class="space-y-2">
                <UButton block color="primary" variant="solid" size="sm" class="justify-start" @click="saveCard">
      ...
      <UCard style="background: var(--panel-background); border: 1px solid var(--border-color)">
        ...
        <div v-if="projectStats" class="space-y-6">
          ...
          <div class="grid grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div class="space-y-2 max-h-48 overflow-y-auto pr-2">
                <div
                  v-for="item in projectStats.assigneeStats"
                  :key="item.name"
                  class="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-700/50 rounded"
                >
                  <div class="flex items-center gap-2">
                    <div
                      class="w-6 h-6 rounded-full bg-gray-300 dark:bg-gray-600 flex items-center justify-center text-xs"
                    >
          ...
          <div>
            ...
            <div class="space-y-2 max-h-48 overflow-y-auto pr-2">
              <div
                v-for="cardTime in projectStats.timePerCard"
                :key="cardTime.title"
                class="flex items-center justify-between p-2 bg-blue-50 dark:bg-blue-900/10 rounded"
              >
          ...
          <!-- Project Info -->
          <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
            <div class="text-sm">Project created {{ projectStats.daysSinceCreation }} days ago</div>
          </div>
        </div>
      </UCard>
    </div>
  ...
  <TemplateSelector v-model="isTemplateModalOpen" :templates="projectTemplates" @select="selectTemplate" />
  ...
  >
    <div class="relative w-full max-w-md rounded-lg shadow-xl">
      <UCard style="background: var(--panel-background); border: 1px solid var(--border-color)">
        ...
        <template #footer>
          <div class="flex justify-end gap-2">
            <UButton color="gray" variant="ghost" @click="isSaveTemplateModalOpen = false"> Cancel </UButton>
            <UButton color="primary" @click="saveAsTemplate"> Save Template </UButton>
          </div>
        </template>
      </UCard>
    </div>
            ...
            >
              ...
              <div class="flex items-center gap-2">
                <USelectMenu
                  ...
                  class="w-40"
                />
                <UButton icon="i-heroicons-trash" size="sm" color="red" variant="ghost" @click="removeMember(member)" />
              </div>
            </div>
            <div
              v-if="!activeProject?.members || activeProject.members.length === 0"
  ...
  >
    <div
      class="relative w-full max-w-3xl rounded-lg shadow-xl"
      style="background: var(--panel-background); border: 1px solid var(--border-color)"
    >
      <UCard>
        <template #header>
        ...
        <div class="space-y-4">
          ...
          <div
            ...
            style="border-color: var(--border-color); background: var(--body-background)"
          >
            ...
            <div
              v-if="column._showPermissions"
              class="mt-4 pt-4 border-t grid grid-cols-3 gap-4"
              style="border-color: var(--border-color)"
            >
              <div>
          ...
          <!-- Add Column Button -->
          <UButton icon="i-heroicons-plus" color="gray" variant="soft" block @click="addColumn"> Add Column </UButton>
        </div>
...
const exportView = async (format: 'png' | 'pdf') => {
  ...
  const toastId = toast.add({
    title: 'Exporting...',
    description: 'Generating ' + format.toUpperCase(),
    color: 'blue',
    timeout: 0,
  });
...
const saveAsTemplate = async () => {
  ...
  const res = await projectStore.saveAsTemplate(
...
const projectStats = computed(() => {
  ...
  const timePerCard = projectCards
    .map((card: any) => {
      let cardMs = 0;
  ...
  projectCards.forEach((card: any) => {
    ...
    if (card.assignee) {
      if (typeof card.assignee === 'object') {
        ...
        // Try to lookup user
        const foundUser = activeProject.value?.members?.find(
          (m: any) => m.userid == card.assignee || m.email == card.assignee,
        );
  ...
  const assigneeStats = Object.values(workloadByAssignee)
    .map((stat) => {
...
const roleOptions = ref<any[]>([{ label: 'Everyone', value: '*' }]);
...
const selectTemplate = (template: any) => {
  ...
  if (template.columns && typeof template.columns === 'string' && template.columns.trim() !== '') {
    try {
      ...
      const jsonString = template.columns.replace(/'/g, '"').replace(/(\w+):/g, '"$1":');
...
const getCurrentUserRole = async () => {
  if (!activeProject.value.members) return;
  await user.loadAuth();
  const userGroup = activeProject.value.members.find((member) => member.userid == user.user?.userid);
...
const hasColumnRestrictions = (column: any) => {
  ...
  return (
    (perms.canDelete && !perms.canDelete.includes('*')) ||
...
const saveCard = async () => {
  ...
  if (editingCard.value.enddate) {
    if (
      activeProject.value.startdate &&
      editingCard.value.enddate.split('T')[0] < activeProject.value.startdate.split('T')[0]
    ) {
      toast.add({
        title: 'Validation Error',
    ...
    if (
      activeProject.value.enddate &&
      editingCard.value.enddate.split('T')[0] > activeProject.value.enddate.split('T')[0]
    ) {
      toast.add({
        title: 'Validation Error',
      ...
      if (editingCard.value.cardid) {
        ...
        try {
          ...
          if (res.data) {
            ...
            const uploadedFile =
              res.data.data && res.data.data.data && Array.isArray(res.data.data.data)
...
const defaultColumns = [
  {
    ...
    icon: '📋',
    color: '#6b7280',
    permissions: { canDelete: [], canMoveOut: [], canMoveIn: [] },
  },
  {
    status: 'todo',
    title: 'To Do',
    ...
    if (newProject && newProject.columns) {
      try {
        ...
        const projectColumns =
          typeof newProject.columns === 'string' ? JSON.parse(newProject.columns) : newProject.columns;
...
const openMembersModal = async () => {
  if (activeProject.value) {
    await Promise.all([loadUsers(), loadProjectMembers()]);
  }
    ...
    return {
      ...
      tasks: tasks.value.filter((t) => {
        ...
        if (typeof t.assignee === 'object') {
          assigned =
            t.assignee.id == member.userid || t.assignee.userid == member.userid || t.assignee.email == member.email;
        } else {
    ...
    if (listGroupBy.value === 'status') {
      ...
      if (card.assignee) {
        if (typeof card.assignee === 'object') {
          ...
          // Try to resolve ID
          const member = activeProject.value?.members?.find(
            (m: any) => m.userid == card.assignee || m.email == card.assignee,
          );
