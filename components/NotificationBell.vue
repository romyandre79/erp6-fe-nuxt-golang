  ...
  <div class="relative overflow-visible">
    ...
    <transition name="dropdown">
      ...
      >
        <!-- Header -->
        <div
          class="px-4 py-3 bg-gradient-to-r from-slate-50 to-slate-100 dark:from-gray-800 dark:to-gray-900 border-b border-slate-200 dark:border-gray-800 flex justify-between items-center"
        >
        ...
        <div class="max-h-72 overflow-y-auto custom-scrollbar">
          <div v-if="notifications.length === 0" class="p-8 text-center">
            <div
              class="w-12 h-12 mx-auto mb-3 rounded-full bg-slate-100 dark:bg-gray-800 flex items-center justify-center"
            >
          ...
          >
            <div class="flex gap-3">
              ...
              >
                <i :class="['fa-solid fa-circle-info text-xs', n.isread ? 'text-gray-400' : 'text-indigo-500']"></i>
              </div>
              <div class="flex-1 min-w-0">
                <div class="flex justify-between items-start mb-0.5">
                  <span class="font-medium text-sm text-gray-800 dark:text-white truncate">{{ n.menuname }}</span>
                  <span class="text-[10px] text-gray-400 dark:text-gray-500 ml-2 flex-shrink-0">{{
                    new Date(n.tododate).toLocaleDateString()
                  }}</span>
                </div>
