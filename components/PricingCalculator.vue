<template>
  <div class="pricing-calculator space-y-24">
    ...
    <section
      id="pricing"
      :class="`py-24 ${theme === 'dark' ? 'bg-slate-950 text-slate-100' : 'bg-white text-slate-900'}`"
    >
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-16">
          <h2 :class="`text-3xl font-bold mb-4 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`">
            {{ $t('ERP.PRICING.TITLE') || 'Calculate Your Price' }}
          </h2>
          <p :class="`${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`">
            {{ $t('ERP.PRICING.SUBTITLE') || 'Select the applications you need and the number of users.' }}
          </p>
        </div>
        ...
        <div class="flex flex-col lg:flex-row gap-8 items-start">
          ...
          <div class="w-full lg:w-2/3">
            <!-- Deployment Mode -->
            <div class="mb-12">
              <h3 :class="`text-lg font-semibold mb-6 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`">
                {{ $t('ERP.PRICING.CHOOSE_DEPLOYMENT') }}
              </h3>
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                >
                  ...
                  <div :class="`text-xs mt-1 ${theme === 'dark' ? 'text-slate-500' : 'text-slate-500'}`">
                    {{ getDeploymentDesc(mode) }}
                  </div>
                </button>
              </div>
            </div>
            ...
            <div v-if="deploymentMode === 'cloud'" class="mb-8">
              <label :class="`block text-sm font-bold mb-3 ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`"
                >Billing Model</label
              >
              <div
                :class="`inline-flex p-1 rounded-xl ${theme === 'dark' ? 'bg-slate-800' : 'bg-slate-100 border border-slate-200'}`"
              >
                <button
              ...
              <p :class="`text-xs mt-2 ${theme === 'dark' ? 'text-slate-500' : 'text-slate-500'}`">
                {{
                  cloudBillingType === 'subscription'
                    ? 'Fixed monthly fee per user + storage.'
                    : 'Pay based on transaction volume. Minimum Rp 500.000/mo includes 5.000 transactions.'
                }}
              </p>
            </div>
            ...
            <div v-show="!(deploymentMode === 'cloud' && cloudBillingType === 'payg')" class="w-full mb-12">
              ...
              <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                >
                  <!-- Checkmark -->
                  <div
                    v-if="selectedApps.includes(app.id)"
                    :class="`absolute top-2 right-2 w-5 h-5 rounded-full flex items-center justify-center text-[10px] ${theme === 'dark' ? 'bg-indigo-600 text-white' : 'bg-blue-600 text-white'}`"
                  >
                    ✓
                  </div>
                  ...
                  <h4
                    :class="`font-medium text-sm h-10 flex items-center justify-center ${getModuleTextClass(app.id)}`"
                  >
                    {{ app.name }}
                  </h4>
                  <p :class="`text-xs mt-2 ${theme === 'dark' ? 'text-slate-500' : 'text-slate-500'}`">
                    Rp {{ formatNumber(app.price) }}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div
              v-if="deploymentMode === 'cloud' && cloudBillingType === 'payg'"
              class="mb-12 bg-opacity-50 p-8 rounded-2xl border"
              :class="theme === 'dark' ? 'bg-slate-900 border-white/10' : 'bg-slate-50 border-slate-100'"
            >
              <h3 :class="`text-lg font-semibold mb-4 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`">
              ...
              <div class="space-y-6">
                ...
                <div>
                  <div class="flex justify-between mb-2">
                    <label :class="`text-sm font-medium ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`"
                      >Accounting & Finance</label
                    >
                    <span :class="`text-xs font-bold ${theme === 'dark' ? 'text-slate-500' : 'text-slate-500'}`"
                      >Rp 5.000 / tx</span
                    >
                  </div>
                  <input
                  ...
                  <div class="flex justify-between mt-1">
                    <input
                      type="number"
                      v-model.number="txAccounting"
                      :class="`w-24 text-sm bg-transparent border-b ${theme === 'dark' ? 'border-slate-700 text-white' : 'border-slate-300 text-slate-900'} focus:outline-none`"
                    />
                ...
                <div>
                  <div class="flex justify-between mb-2">
                    <label :class="`text-sm font-medium ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`"
                      >Inventory / Procurement / Order</label
                    >
                    <span :class="`text-xs font-bold ${theme === 'dark' ? 'text-slate-500' : 'text-slate-500'}`"
                      >Rp 3.000 / tx</span
                    >
            ...
            <div
              v-show="!(deploymentMode === 'cloud' && cloudBillingType === 'payg')"
              class="mb-12 bg-opacity-50 p-8 rounded-2xl border"
              :class="theme === 'dark' ? 'bg-slate-900 border-white/10' : 'bg-slate-50 border-slate-100'"
            >
              <h3 :class="`text-lg font-semibold mb-4 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`">
                {{ $t('ERP.PRICING.USERS_COUNT') }}
              </h3>
              ...
              <div class="space-y-4">
                ...
                <div class="flex items-center justify-between">
                  <label :class="`text-sm font-medium ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`">{{
                    $t('ERP.PRICING.USERS_OPERATIONAL')
                  }}</label>
                  <div class="flex items-center space-x-4">
                    <button @click="usersOperational > 0 ? usersOperational-- : null" :class="getUserBtnClass()">
                      -
                    </button>
              ...
              <p :class="`text-xs mt-4 italic ${theme === 'dark' ? 'text-slate-500' : 'text-slate-500'}`">
                {{ $t('ERP.PRICING.USERS_NOTE') }}
              </p>
            </div>
            ...
            >
              <div class="flex items-center justify-between mb-2">
                <h3 :class="`text-lg font-semibold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`">
                  {{ $t('ERP.PRICING.STORAGE') }}
                </h3>
                <span :class="`font-bold ${theme === 'dark' ? 'text-indigo-400' : 'text-blue-600'}`"
                  >{{ storageSize }} GB</span
                >
              </div>
              <input
                ...
                :class="`w-full h-2 rounded-lg appearance-none cursor-pointer ${theme === 'dark' ? 'bg-slate-700 accent-indigo-500' : 'bg-slate-200 accent-blue-600'}`"
              />
              <p :class="`text-sm mt-2 ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`">
                {{ $t('ERP.PRICING.STORAGE_NOTE') }}
              </p>
            </div>
            ...
            <div class="mt-12">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                >
                  <label
                    :class="`block font-semibold mb-2 ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`"
                    >{{ $t('ERP.PRICING.CONSULTATION') }}</label
                  >
                  ...
                  <p :class="`text-xs mt-2 ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`">
                    Rate: Rp {{ formatNumber(CONSULTATION_RATE) }} / hr / module
                  </p>
                </div>
                <div
                  class="p-6 rounded-2xl border"
                ...
                >
                  ...
                  <p :class="`text-xs mt-2 ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`">
                    Rate: Rp {{ formatNumber(TRANSPORT_RATE) }} / day
                  </p>
                </div>
              </div>
            </div>
                ...
                <div v-for="(workflows, category) in workflowCategories" :key="category">
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    >
                      ...
                      <div class="flex-1">
                        <div :class="`font-medium text-sm ${theme === 'dark' ? 'text-slate-200' : 'text-slate-800'}`">
                          {{ wf.name }}
                        </div>
                        <div class="text-xs text-slate-500">Rp {{ formatNumber(wf.price) }} / mo</div>
                      </div>
                      <div
                        v-if="selectedWorkflows.includes(wf.id)"
                        :class="`font-bold ${theme === 'dark' ? 'text-indigo-400' : 'text-blue-600'}`"
                      >
                        ✓
                      </div>
                    </div>
          ...
          <div class="w-full lg:w-1/3 lg:sticky lg:top-24">
            <div
              :class="`rounded-2xl shadow-xl border p-8 ${theme === 'dark' ? 'bg-slate-900 border-white/10' : 'bg-white border-slate-200'}`"
            >
              <h3
                :class="`text-lg font-bold mb-6 border-b pb-4 ${theme === 'dark' ? 'text-white border-white/5' : 'text-slate-900 border-slate-100'}`"
              >
                {{ $t('ERP.PRICING.ESTIMATED_COST') }}
              </h3>

              <div class="space-y-3 mb-6">
                <!-- Deployment Info -->
                <div
                  :class="`flex justify-between text-sm font-medium p-2 rounded border ${theme === 'dark' ? 'bg-slate-800 border-white/5 text-slate-400' : 'bg-slate-50 border-slate-100 text-slate-600'}`"
                >
                  <span>{{ $t('ERP.PRICING.DEPLOYMENT') }}</span>
                  <div class="text-right">
                    <span :class="`block capitalize ${theme === 'dark' ? 'text-indigo-400' : 'text-blue-600'}`">{{
                      deploymentModeTitle
                    }}</span>
                    <span
                      v-if="deploymentMode === 'cloud'"
                      :class="`text-xs ${theme === 'dark' ? 'text-slate-500' : 'text-slate-500'}`"
                      >{{ cloudBillingType === 'subscription' ? 'Subscription' : 'Pay As You Go' }}</span
                    >
                  </div>
                </div>
                ...
                <template v-if="!(deploymentMode === 'cloud' && cloudBillingType === 'payg')">
                  <div
                    :class="`flex justify-between text-sm ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`"
                  >
              ...
              <div
                v-if="oneTimeFee > 0"
                :class="`pt-4 border-t mb-4 ${theme === 'dark' ? 'border-white/10' : 'border-slate-100'}`"
              >
                <div
                  :class="`flex justify-between text-sm mb-1 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`"
                >
                ...
                <div
                  v-if="deploymentMode !== 'cloud'"
                  :class="`flex justify-between text-sm font-bold mt-2 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`"
                >
                  <span>One-Time License/Source</span>
                  <span>Rp {{ formatNumber(oneTimeFee - implementationFee - transportFee) }}</span>
                </div>
                <div
                  :class="`flex justify-between text-lg font-bold mt-2 pt-2 border-t border-dashed ${theme === 'dark' ? 'text-indigo-400 border-white/10' : 'text-slate-800 border-slate-200'}`"
                >
              ...
              <div :class="`pt-6 border-t ${theme === 'dark' ? 'border-white/10' : 'border-slate-100'}`">
                <p :class="`text-xs mb-1 ${theme === 'dark' ? 'text-slate-500' : 'text-slate-500'}`">
                  {{ $t('ERP.PRICING.TOTAL_MONTHLY') }}
                </p>
                <div :class="`text-3xl font-extrabold mb-2 ${theme === 'dark' ? 'text-white' : 'text-blue-600'}`">
                  Rp {{ formatNumber(monthlyRecurring) }}
                </div>
                <p
                  v-if="deploymentMode === 'cloud' && cloudBillingType === 'subscription'"
                  class="text-xs text-slate-500"
                >
                  Billed Annually. Calcualted based on Weighted Users.
                </p>
                <p v-else-if="deploymentMode === 'cloud' && cloudBillingType === 'payg'" class="text-xs text-slate-500">
                  Includes 5.000 transactions. Excess billed per module rate.
                </p>
    ...
    <section
      id="demo"
      ref="demoSection"
      :class="`py-24 ${theme === 'dark' ? 'bg-gradient-to-b from-slate-900 to-slate-950 border-t border-white/5' : 'bg-slate-50 border-t border-slate-200'}`"
    >
      <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div
          :class="`rounded-3xl shadow-xl p-10 md:p-16 border ${theme === 'dark' ? 'bg-slate-800/50 backdrop-blur-xl border-white/10' : 'bg-white border-slate-100'}`"
        >
          ...
          <form @submit.prevent="submitDemo" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="col-span-1">
                <label
                  :class="`block text-sm font-medium mb-2 ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`"
                  >{{ $t('ERP.DEMO.NAME') }}</label
                >
              ...
              <div class="col-span-1">
                <label
                  :class="`block text-sm font-medium mb-2 ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`"
                  >{{ $t('ERP.DEMO.PHONE') }}</label
                >
                <input
                  v-model="form.phone"
                  type="tel"
                  required
                  :class="getFormInputClass()"
            ...
            <p
              v-if="message"
              :class="`text-center text-sm font-medium ${messageType === 'success' ? (theme === 'dark' ? 'text-green-400' : 'text-green-600') : theme === 'dark' ? 'text-red-400' : 'text-red-600'}`"
            >
              {{ message }}
            </p>
          </form>
...
const avgTxRate = computed(() => {
  ...
  const totalVal =
    txAccounting.value * TX_RATES.accounting +
...
const baseMonthlyCost = computed(() => {
  ...
  // Subscription Logic
  const userFactor =
    usersAdmin.value * USER_WEIGHTS.admin +
...
const toggleWorkflow = (wf) => {
  if (selectedWorkflows.value.includes(wf.id))
...
const getDeploymentClass = (mode) => {
  ...
  if (props.theme === 'dark') {
    return isActive
      ? 'border-indigo-600 bg-indigo-900/20 ring-2 ring-indigo-500/50'
...
const getModuleClass = (id) => {
  ...
  if (props.theme === 'dark') {
    return isSelected
      ? 'border-indigo-600 bg-indigo-900/20 shadow-[0_0_15px_-5px_rgba(99,102,241,0.3)]'
      : 'border-slate-800 bg-slate-900 text-slate-400 hover:border-slate-700';
  } else {
