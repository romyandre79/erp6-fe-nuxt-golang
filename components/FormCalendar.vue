...
const getPriorityColorClass = (priority: string) => {
  ...
  if (p === 'high' || p === 'urgent')
    return 'bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-300 border-red-100 dark:border-red-800';
  if (p === 'medium')
    return 'bg-orange-50 text-orange-700 dark:bg-orange-900/20 dark:text-orange-300 border-orange-100 dark:border-orange-800';
...
<template>
  ...
  >
    ...
    <div class="flex-1 overflow-auto p-4">
      <div
        class="grid grid-cols-7 gap-px bg-gray-200 dark:bg-gray-700 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden h-full"
      >
        <!-- Weekday Headers -->
        <div
          v-for="day in ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']"
          :key="day"
          class="bg-gray-100 dark:bg-gray-800/50 p-2 text-center text-xs font-semibold text-gray-500 dark:text-gray-400"
        >
          {{ day }}
        </div>
        ...
        >
          <div
            class="text-right text-xs font-medium mb-1"
