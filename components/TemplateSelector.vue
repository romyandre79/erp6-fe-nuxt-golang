<template>
  ...
  >
    <div class="relative w-full max-w-4xl rounded-lg shadow-xl max-h-[90vh] flex flex-col">
      <UCard style="background: var(--panel-background); border: 1px solid var(--border-color);" class="flex flex-col max-h-[90vh]">
        <template #header>
          ...
          <div class="grid grid-cols-3 gap-4">
            ...
            >
              ...
              <div class="mt-3 text-xs text-gray-500">
                {{ getColumnCount(template) }} columns
              </div>
            </div>
          </div>
...
const filteredTemplates = computed(() => {
  ...
  return props.templates.filter((template: any) => 
...
const getColumnCount = (template: any): number => {
  ...
  try {
    ...
    if (typeof template.columns === 'string') {
      ...
      let jsonString = template.columns
        .replace(/'/g, '"')
        .replace(/(\w+):/g, '"$1":');
