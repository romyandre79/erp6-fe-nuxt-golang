...
const parsedSchema = reactive(typeof props.schema === 'string' ? JSON.parse(props.schema) : props.schema);
            ...
            component.details?.map((detail: any, idx: number) =>
              h('div', { key: idx, class: 'mb-3' }, [
                ...
                detail.components?.map((child: any) =>
                  ['table', 'details'].includes(child.type) ? renderContainer(child) : renderComponent(child),
                ),
              ]),
            ),
          ]);
...
<template>
  <div class="w-full">
    ...
    <div
      ...
      v-if="formType.toUpperCase() == 'MASTER' || formType.toUpperCase() == 'MASTER-DETAIL'"
    >
      <button
        v-if="isButtonExist('new')"
      ...
      <!-- Input file hidden -->
      <input ref="fileInput" type="file" class="hidden" accept=".xls,.xlsx" @change="handleFileChange" />
      ...
      <button
        v-if="isButtonExist('download template')"
        class="px-4 py-2 rounded text-white bg-gray-600 hover:bg-gray-700 transition"
        @click="downTemplate()"
      >
