<template>
  ...
  >
    ...
    <div
      v-if="!isRoot"
      class="w-2 h-2 bg-gray-400 rounded-full absolute -top-3 left-1/2 transform -translate-x-1/2"
    ></div>
    ...
    >
        ...
        <div v-if="employees.length > 0" class="mt-3 pt-2 border-t border-gray-100">
          ...
          <div class="flex -space-x-1 overflow-hidden">
            <template v-for="(emp, i) in employees.slice(0, 4)" :key="i">
              ...
              <div
                v-else
                class="inline-block h-6 w-6 rounded-full ring-2 ring-white bg-gray-200 flex items-center justify-center text-[10px]"
              >
                {{ (emp.name || '?').charAt(0) }}
              </div>
            </template>
            <div
              v-if="employees.length > 4"
              class="inline-block h-6 w-6 rounded-full ring-2 ring-white bg-gray-100 flex items-center justify-center text-[10px] text-gray-500 font-medium"
            >
              +{{ employees.length - 4 }}
            </div>
          </div>
      ...
      <div
        class="absolute bottom-1 right-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity flex gap-1"
      >
        <button
    ...
    <div
      v-if="hasChildren"
      class="w-2 h-2 bg-gray-400 rounded-full absolute -bottom-3 left-1/2 transform -translate-x-1/2"
    ></div>
  </div>
</template>
